...
parser.add_argument("--min_frac", type=float, required=True)
...
for run in range(num_runs):
    print(f"Minority fraction {min_frac}, Run {run + 1}/{num_runs}")
...
pkl_path = f"save_results/minfrac_{min_frac:.2f}_eps{target_epsilon}.pkl"
...
csv_path = f"save_results/minfrac_{min_frac:.2f}_eps{target_epsilon}.csv"
summary_df.to_csv(csv_path, index=False)

print(f"Completed min_frac = {min_frac}")
