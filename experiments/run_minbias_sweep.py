...
parser.add_argument("--min_bias", type=float, required=True)
...
for run in range(num_runs):
    print(f"Min bias {min_bias}, Run {run + 1}/{num_runs}")
...
pkl_path = f"save_results/minbias_{min_bias:+.2f}_eps{target_epsilon}.pkl"
...
csv_path = f"save_results/minbias_{min_bias:+.2f}_eps{target_epsilon}.csv"
summary_df.to_csv(csv_path, index=False)

print(f"Completed min_bias = {min_bias}")
